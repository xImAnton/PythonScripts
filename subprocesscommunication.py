...
class ProcessCommunication:
    ...
    """
    def __init__(self, command: str, cwd: str = ".", on_output: Callable = lambda line: print(line, end=""), on_close: Callable = lambda: print("process communication ended")):
        """
    ...
    def write_stdin(self, cmd: str, newline: bool = True, flush: bool = True) -> None:
        """
        writes a line to process stdin and flushes it
        :param cmd: the command to send
        :param newline: whether a newline should be added at the end of the line
        :param flush: whether the stdin stream should be flushed
        """
        if not self.running:
            return
        self.process.stdin.write(cmd.encode("ascii") + b'\n' if newline else cmd.encode("ascii"))
        if flush:
